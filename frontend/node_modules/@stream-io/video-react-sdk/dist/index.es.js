import { SfuModels, disposeOfMediaStream, CallingState, OwnCapability, name, NoiseCancellationSettingsModeEnum, VisibilityState, Browsers, paginatedLayoutSortPreset, combineComparators, screenSharing, speakerLayoutSortPreset, CallTypes, defaultSortPreset, setSdkInfo } from '@stream-io/video-client';
...
const ParticipantView = forwardRef(function ParticipantView({ participant, trackType = 'videoTrack', muteAudio, refs: { setVideoElement, setVideoPlaceholderElement } = {}, className, VideoPlaceholder, ParticipantViewUI = DefaultParticipantViewUI, }, ref) {
    ...
    return (jsx("div", { "data-testid": "participant-view", ref: (element) => {
        ...
        }, className: clsx('str-video__participant-view', isDominantSpeaker && 'str-video__participant-view--dominant-speaker', isSpeaking && 'str-video__participant-view--speaking', !hasVideo && 'str-video__participant-view--no-video', !hasAudio && 'str-video__participant-view--no-audio', className), children: jsxs(ParticipantViewContext.Provider, { value: participantViewContextValue, children: [!isLocalParticipant && !muteAudio && (jsxs(Fragment, { children: [hasAudio && (jsx(Audio, { participant: participant, trackType: "audioTrack" })), hasScreenShareAudio && (jsx(Audio, { participant: participant, trackType: "screenShareAudioTrack" }))] })), jsx(Video$1, { VideoPlaceholder: VideoPlaceholder, participant: participant, trackType: trackType, refs: videoRefs, autoPlay: true }), isComponentType(ParticipantViewUI) ? (jsx(ParticipantViewUI, {})) : (ParticipantViewUI)] }) }));
});
...
const hasScreenShare$1 = (p) => !!p?.publishedTracks.includes(SfuModels.TrackType.SCREEN_SHARE);
...
const [major, minor, patch] = ("0.6.19" ).split('.');
setSdkInfo({
